...
import os
import sys
from plothelpers_V0.plothelpers import InitialiseFigure, InitialiseSubPlot, GenerateAxes
import time
...
COLOR_WHEEL = ['b', 'r', 'g', 'purple', 'teal', 'orange']

logging.basicConfig(level=logging.INFO)

plot_cfg_path = abspath("../config/plt_config_params.json")
...
# Read in data
    ...
    for i, plot in enumerate(plot_params['plots']):
        ...
        for k, subplot in enumerate(plot['subplots']):
            ...
            pscad_index = subplot['pscad_index']
            y_min = None
            y_max = None
            # logging.info(f'pscad_index: {pscad_index}')
            if pscad_index is not None:
                upper_df = df.iloc[:, pscad_index] * 1.1
                upper_df.columns = upper_df.columns + ' + 10%'
                lower_df = df.iloc[:, pscad_index] * 0.9
                lower_df.columns = lower_df.columns + ' - 10%'
            ...
            if y_min is not None and y_max is not None:
                ...
                if pscad_index is not None:
                    upper_df.plot(ax=axes[k], grid='on', xlim=x_lims, ylim=y_lims)
                    lower_df.plot(ax=axes[k], grid='on', xlim=x_lims, ylim=y_lims)
            else:
                df.iloc[:, y_axis].plot(ax=axes[k], grid='on', xlim=x_lims)
                if pscad_index is not None:    
                    upper_df.plot(ax=axes[k], grid='on', xlim=x_lims)
                    lower_df.plot(ax=axes[k], grid='on', xlim=x_lims)
